...
local Library = {
    ...
    Scheme = {
        ...
        AccentColor = Color3.fromRGB(100, 54, 172),
...
-- [[ Enhancements Added by ChatGPT ]] --

-- Add UI sounds
local UISound = Instance.new("Sound", SoundService)
UISound.SoundId = "rbxassetid://6026984224"
UISound.Volume = 0.3

local function PlayUISound()
    if UISound.IsLoaded then
        UISound:Play()
    else
        UISound.Loaded:Connect(function()
            UISound:Play()
        end)
    end
end

-- Animate Hover for Buttons
local function AnimateHover(button)
    local defaultColor = Library.Scheme.MainColor
    local hoverColor = Library:GetBetterColor(defaultColor, 10)
...
local function RemoveBlur()
    local blur = game:GetService("Lighting"):FindFirstChild("UILibraryBlur")
    if blur then blur:Destroy() end
end

-- Theme Toggle (Light/Dark)
function Library:ToggleTheme()
    Library.IsLightTheme = not Library.IsLightTheme
    if Library.IsLightTheme then
        Library.Scheme = {
            BackgroundColor = Color3.fromRGB(245, 245, 245),
            MainColor = Color3.fromRGB(220, 220, 220),
            AccentColor = Color3.fromRGB(100, 54, 172),
            OutlineColor = Color3.fromRGB(200, 200, 200),
            FontColor = Color3.new(0.1, 0.1, 0.1),
            Font = Font.fromEnum(Enum.Font.Code),
            Red = Color3.fromRGB(255, 50, 50),
            Dark = Color3.new(0.2, 0.2, 0.2),
            White = Color3.new(1, 1, 1),
        }
    else
        Library.Scheme = {
